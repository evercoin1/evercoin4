...
// Copyright (c) 2011-2012 EVERCOIN Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #EVERCOINTEST0\r");
            Send(hSocket, "WHO #EVERCOINTEST0\r");
        } else {
            // randomly join #EVERCOIN00-#EVERCOIN99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #EVERCOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #EVERCOIN%02d\r", channel_number).c_str());
        }

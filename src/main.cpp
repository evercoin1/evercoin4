...
uint256 hashGenesisBlock("0xfe93bd446fefed9ee99942ebfe407a19a9d590b648c483dd688b4d4653df489d");
...
{
    // EVERCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if (nHeight == 1)
		return 63000000 * COIN;
	
	// Subsidy halving
    nSubsidy >>= (nHeight / 147000);
...
static const int64 nTargetSpacing = 1.67 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 1.67 * 60;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * 1.67 * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * 1.67 * 2.8;
...
{
    // EVERCOIN: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ever genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xd7, 0xca, 0xaa, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EVERCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EVERCOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EVERCOINMiner(CWallet *pwallet)
{
    printf("EVERCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("EVERCOIN-miner");
    ...
    try { loop {
        ...
        printf("Running EVERCOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EVERCOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EVERCOINMiner, pwallet));
}
